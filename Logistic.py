...
def show_page():
    ...
    # Buat tabel barang_keluar dengan struktur baru
    c.execute("""
    CREATE TABLE IF NOT EXISTS barang_keluar (
        ...
        NO_PO TEXT DEFAULT '',
        NO_DO TEXT DEFAULT '',
        CREATED_AT TEXT DEFAULT (strftime('%d-%m-%Y %H:%M', 'now', '+7 hours'))
    )
    """)

    # CEK DAN TAMBAH KOLOM JIKA BELUM ADA
    try:
        c.execute("ALTER TABLE barang_keluar ADD COLUMN NO_PO TEXT DEFAULT ''")
    except sqlite3.OperationalError:
        pass  # Kolom sudah ada

    try:
        c.execute("ALTER TABLE barang_keluar ADD COLUMN NO_DO TEXT DEFAULT ''")
    except sqlite3.OperationalError:
        pass  # Kolom sudah ada
    ...
    if menu == "📦 Barang Masuk":
        ...
        if st.button("Simpan"):
            if supplier and item:
                c.execute("INSERT INTO barang_masuk (TANGGAL, SUPPLIER, BARANG, JUMLAH) VALUES (?, ?, ?, ?)",
